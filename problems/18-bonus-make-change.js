...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
    ...
    if (target < minCoin) return results;

    for (let i = 0; i < coins.length; i++) {
        let current = coins[i];
        if (current <= target) {
            results.push(current);
            return results.concat(greedyMakeChange(target - current, coins));
        }
    }
}
...
const makeBetterChange = (target, coins = [25, 10, 5, 1]) => {
    let toReturn = null;
    if (target < 0) return toReturn;
    if (target === 0) return [];

    for (let i = 0; i < coins.length; i++) {
        const sum = target - coins[i];
        const results = makeBetterChange(sum, coins.slice(i));

        if (results !== null) {
            const array = [...results, coins[i]];
            if (toReturn === null || array.length < toReturn.length) {
                toReturn = array;
            }
        }
    }
